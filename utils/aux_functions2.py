...
super_predictor = dlib.shape_predictor(path_to_dlib_model)
...
def mask_image(image_path, mask_type='surgical'):
    # Read the image
    image = cv2.imread(image_path)
    original_image = image.copy()
    # gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    gray = image
    face_locations = dlib.get_frontal_face_detector()(gray, 1)

    tqdm.write("Faces found: {:2d}".format(len(face_locations)))
    # Process each face in the image
    masked_images = []
    for (i, face_location) in enumerate(face_locations):
        shape = super_predictor(gray, face_location)
        ...
        image, mask_binary = mask_face(image, face_location, six_points_on_face, angle, type=mask_type)
...
def mask_from_image(image, mask_type='surgical'):
    # Read the image
    original_image = image.copy()
    # gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    gray = image
